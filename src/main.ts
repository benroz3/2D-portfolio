...
kbm.loadSprite("spritesheet", "./spritesheet.png", {
  sliceX: 39,
  sliceY: 31,
  anims: {
    "idle-down": 936,
...
kbm.scene("main", async () => {
  ...
  const map = kbm.add([kbm.sprite("map"), kbm.pos(0), kbm.scale(scaleFactor)]);
  const player = kbm.make([
    kbm.sprite("spritesheet", { anim: "idle-down" }),
    kbm.area({
  ...
  kbm.onUpdate(() => {
    kbm.camPos(player.pos.x, player.pos.y + 100);
  });

  kbm.onMouseDown((mouseBtn) => {
